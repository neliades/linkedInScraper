...
const {findCompaniesWithoutInfo} = require('../db/findCompaniesWithoutInfo.js');
const {addListOfCompanies} = require('../db/addCompany.js');
...
const addToDb = async function (searchURL) {
  ...
  let newCompanies = await findCompaniesWithoutInfo('companiesFROMusers');
  await addListOfCompanies(newCompanies);
  await console.log('\n\n\n\ndone\n\n\n\n');
}
